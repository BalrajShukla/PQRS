...
let themeToggle, themeIcon, themeLabel, modeSelect, doiGroup, doiInput, fileInput;
let fileLabel, runBtn, btnText, btnSpinner, statusMessage, batchInfo;
let resultsBody, exportCsvBtn, apiKeyInput, saveKeyBtn;

// Initialize App ONLY after HTML is fully drawn on screen
window.addEventListener('DOMContentLoaded', async () => {
  ...
  doiGroup = document.getElementById('doiGroup');
  ...
  runBtn?.addEventListener('click', executeExtraction);
  exportCsvBtn?.addEventListener('click', downloadCSV);

  // 4. Load Scopus and Embase Databases
...
14. reporting_guidelines: Scan Methodology/Text for ANY statement of adherence to reporting standards or frameworks (e.g., PRISMA, CARE, STROBE, CONSORT, or author-developed frameworks like "guidelines developed by Boateng et al."). Output the exact framework name or citation. If none, state "Not reported".
...
17. protocol_registration: Scan text and references for protocol registrations (e.g., OSF, PROSPERO, ClinicalTrials.gov). Reconstruct split DOIs caused by line breaks (e.g., "10.17605/OSF.IO/TVUKC"). Extract full DOI or URL.
...
`;
  ...
  rawJsonText = rawJsonText.replace(/^```json\s*/im, '').replace(/\s*```$/im, '').trim();
